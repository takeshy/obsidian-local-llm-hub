...
import { AGENT_PLUGIN_ROOT, agentPluginAbsolutePaths, installAgentPlugin, parseAgentPluginMcp, previewAgentPlugin, uninstallAgentPlugin } from "src/core/agentPlugins";
...
export function displayAgentPluginSettings(containerEl: HTMLElement, ctx: SettingsContext): void {
  ...
  new Setting(containerEl).setName("GitHub repository").setDesc("Owner/repository or a GitHub URL").addText(text => text.setPlaceholder("Owner/repository").onChange(value => { repository = value; })).addButton(button => button.setButtonText("Preview and install").setCta().onClick(() => { void (async () => {
    ...
    try {
      ...
      const managed = bytes ? parseAgentPluginMcp(new TextDecoder().decode(bytes), preview.manifest.name, paths.root, paths.data).servers.map(v => mergeServer(v, prior.get(v.agentPlugin!.serverName))) : [];
      let metadata: AgentPluginInstall | null = null;
      new AgentPluginInstallModal(plugin.app, preview, managed, async () => {
        for (const server of plugin.settings.mcpServers.filter(v => v.agentPlugin?.pluginName === preview.manifest.name)) await plugin.mcpManager.disconnectServer(server.id);
