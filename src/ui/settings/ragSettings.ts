...
export function displayRagSettings(containerEl: HTMLElement, ctx: SettingsContext): void {
  ...
  new Setting(containerEl)
    ...
    .addText((text) => {
      text
          .setPlaceholder("http://localhost:11434") // eslint-disable-line obsidianmd/ui/sentence-case -- URL placeholder
        .setValue(ragConfig.embeddingBaseUrl)
        .onChange(async (value) => {
          plugin.settings.ragConfig = { ...ragConfig, embeddingBaseUrl: value };
          await plugin.saveSettings();
        });
