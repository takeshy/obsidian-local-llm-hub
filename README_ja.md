...
ヘッダーからサイドバーの拡大・縮小と「ノートとして保存」を実行できます。手動保存は`YYYYMMDD-HHmmss_チャットタイトル.md`形式で、履歴用メタデータを含まないコンパクトな本文を出力し、同じチャットを再保存すると上書きします。

![RAG 付きチャット](docs/images/chat_with_rag.png)

### 音声入力と読み上げ

**回答の読み上げ** — Vault ツールメニューの「回答を自動で読み上げる」をONにします。ON の間は入力欄の上にチップが出て、✕ でOFFにできます。各回答のバブルにもスピーカーボタンがあり、その回答だけを読み上げたり、途中で止めたりできます。読み上げ速度はチェックボックスのすぐ下でも、設定（設定 → チャット）でも変更できます（0.5倍〜5倍。実際にどこまで速くなるかは音声次第で、Windows の音声はおおむね4倍以上では変わりません）。読み上げがONの間は、Markdown や見出し・箇条書き・コード・URL を避けて短く話すよう、システムプロンプトでモデルに指示します。

**音声入力の自動送信** — 設定の「音声入力を自動送信」をONにすると、貼り付け・入力されたテキストが送信フレーズ（日本語の既定は「送信して」、言語ごとに変更可）で終わったときに送信します。OS の音声入力や Aqua Voice などで使えます。

**音声会話** — [speech-popup](https://github.com/takeshy/speech-popup) をインストールすると、送信ボタンの上にマイクボタンが出ます。押すと常にポップアップを開くだけなので、閉じてしまっても1回押せば戻ります。会話中は:

- 話してポップアップから送ると、そのまま質問として送信されます。speech-popup が貼り付けに `⟦voice-chat⟧` という印を付ける（`show --append`）ので、普通のクリップボード貼り付けは送信されず入力欄に入ります。回答の生成中に届いた場合も入力欄に入るので、文章が消えません。
- 回答は読み上げられ（会話の開始時に読み上げがONになります）、読み上げが終わってから約2.5秒後にポップアップが再表示されます。自分の読み上げをマイクが拾わないための間です。
- 読み上げチップの✕で読み上げだけを切っても会話は続きます（回答のたびにポップアップは開きます）。
...
- Flatpak 版 Obsidian では、最初に一度 `flatpak override --user --talk-name=org.freedesktop.Flatpak md.obsidian.Obsidian` でホストへのポータルを許可し、上のコマンド欄にホスト側のフルパス（例: `/home/you/.local/bin/speech-popup`）を入れてください。コマンドは `flatpak-spawn --host` 経由で実行します。Snap 版には同等の仕組みが無いので、AppImage 版か Flatpak 版を使ってください。
...
**Vault: 読み取り専用** は検索・読み取りだけを許可し、組み込みツールによるファイルやフォルダの作成・編集・削除・名前変更を禁止します。チャットのVaultツールメニューやスラッシュコマンドで選択できます。外部MCPやスキルの権限は別途設定します。
...
1. 設定で RAG を有効化
2. 埋め込みモデルを取得・選択
3. RAGインデックス対象フォルダを設定（省略時はVault全体。Vaultツールのアクセス範囲は制限しません）
...
**ツール実行の承認：** 初期状態では実行前にサーバー名・ツール名・実行引数を確認し、「今回だけ許可」「このツールを常に許可」「拒否」を選びます。画面を閉じた場合も拒否します。サーバー設定の「常に承認」をONにすると確認を省略します。許可リストからツールを削除して保存すると再び確認が必要になります。MCP設定に未登録のサーバーは今回だけの許可に対応します。

ワークフローの `command` ノードは `confirm: "false"` で自動実行を含むMCP承認を省略できます。`vaultTools: "readOnly"` で組み込みVaultツールを検索・読み取りに制限できます。既定値は従来どおり `noSearch` です。どちらもノード編集画面で設定できます。ローカル版はcommandノードからMCPを呼び出す構成で、単独の `mcp` ノードはありません。

![MCP & 暗号化設定](docs/images/setting_mcp_server_and_encryption.png)

### ワークスペース設定

**LLMのVaultツール許可フォルダ** で、LLMによる自動的なVault操作を許可するフォルダを指定できます。空欄の場合はVault全体を許可します。

チャット履歴の自動保存には最大件数を設定できます。`0`は無制限です。既存ユーザーは無制限、新規インストールは100件が初期値です。

### Chat 設定

**チャットの手動保存先**で「ノートとして保存」のVault相対フォルダを指定します。空欄の場合はVault直下です。
